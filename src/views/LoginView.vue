  ...
  <div class="login">
    ...
    <div v-if="state.type === 'login'" class="login-body login">
      <VanForm @submit="onSubmit">
        <VanField v-model="state.username" name="username" label="用户名" placeholder="用户名" :rules="[{
          required: true,
          message: '请填写用户名'
        }]" autocomplete="off" />
        <VanField v-model="state.password" name="password" label="密码" type='password' placeholder="密码" :rules="[{
          required: true,
          message: '请填写密码'
        }]" />
        <VanField center clearable label="验证码" placeholder="输入验证码" v-model="state.verify">
          <template #button>
        ...
        <div style="margin: 16px;">
          ...
          <VanButton color="#1baeae" native-type="submit" round block>
            登录
          </VanButton>
        </div>
      </VanForm>
    ...
    <div v-else class="login-body register">
      <VanForm @submit="onSubmit">
        <VanField v-model="state.username1" name="username1" label="用户名" placeholder="用户名"
          :rules="[{ required: true, message: '请填写用户名' }]" />
        <VanField v-model="state.password1" name="password1" label="密码" placeholder="密码"
          :rules="[{ required: true, message: '请填写密码' }]" />
        <VanField label="验证码" center name="" placeholder="请输入验证码" v-model="state.verify">
          <template #button>
            <VueImageVerify ref="verifyRef" />
          </template>
        </VanField>
